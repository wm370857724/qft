...
def binary_to_quaternary_optimized(binary_data):
    """优化的二进制到四进制转换"""
    if not binary_data:
        return ''
...
def quaternary_to_pixels_optimized(quaternary_str, width, height):
...
def tar_to_bmp_optimized(tar_path, bmp_path, width=2540, height=1470):
    """优化的TAR到BMP转换函数"""
    # 读取二进制数据
...
def process_single_file(args):
    ...
    try:
        ...
        return "Converted {0} to {1}".format(tar_file_path, bmp_file_path)
    except Exception as e:
        return "Error converting {0}: {1}".format(tar_file_path, str(e))

def convert_folder_optimized(folder_path, width=2540, height=1470, use_multiprocessing=True):
    """优化的文件夹转换函数，支持多进程"""
    ...
    for filename in os.listdir(folder_path):
        ...
        if match:
            ...
            bmp_filename = "output.{0}.bmp".format(match.group(1))
    ...
    if use_multiprocessing and len(files_to_process) > 1:
        ...
        print("Using {0} processes to convert {1} files...".format(num_processes, len(files_to_process)))
