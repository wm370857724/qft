...
def find_closest_color_optimized(pixel: Tuple[int, int, int]) -> str:
    """优化的颜色匹配函数，使用numpy进行向量化计算"""
    pixel_array = np.array(pixel)
    # 计算与所有颜色的距离
    distances = np.sum((COLOR_ARRAYS - pixel_array) ** 2, axis=1)
    # 找到最小距离的索引
    min_idx = np.argmin(distances)
    return list(COLOR_MAP.values())[min_idx]

def find_closest_color_by_rule_optimized(pixel: Tuple[int, int, int]) -> str:
    """优化的基于规则的颜色匹配"""
    r, g, b = pixel
    ...
    if r > 180 and g > 180 and b > 180:
        ...
        # 对于其他情况，使用优化的颜色匹配
        return find_closest_color_optimized(pixel)
...
def bmp_to_tar_optimized(bmp_path: str, tar_path: str):
    """优化的BMP到TAR转换函数"""
    # 打开并裁剪图像
    original_image = Image.open(bmp_path)
    left, top, width, height = 10, 62, 1900, 950
    ...
    for pixel in pixels_flat:
        digit = find_closest_color_by_rule_optimized(tuple(pixel))
        if digit == '4':
            break
...
if __name__ == '__main__':
    # 使用优化版本
    bmp_to_tar_optimized('example/example.bmp', 'recovered_example_optimized.tar')
